...
struct std::hash<Hypnos::Eval::NNUE::Network<ArchT, FeatureTransformerT>> {
    std::size_t operator()(
      const Hypnos::Eval::NNUE::Network<ArchT, FeatureTransformerT>& network) const noexcept {
        return network.get_content_hash();
    }
};

template<>
struct std::hash<Hypnos::Eval::NNUE::Networks> {
    std::size_t operator()(const Hypnos::Eval::NNUE::Networks& networks) const noexcept {
        std::size_t h = 0;
        Hypnos::hash_combine(h, networks.big);
        Hypnos::hash_combine(h, networks.small);
        return h;
    }
};
