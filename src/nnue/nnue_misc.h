...
template<>
struct std::hash<Hypnos::Eval::NNUE::EvalFile> {
    std::size_t operator()(const Hypnos::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        Hypnos::hash_combine(h, evalFile.defaultName);
        Hypnos::hash_combine(h, evalFile.current);
        Hypnos::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
